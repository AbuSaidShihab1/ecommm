...
const Breviews = () => {
   const navigate=useNavigate();
     const {activesidebar,setactivesidebar,activetopbar,setactivetopbar}=useContext(Contextapi);
         ...
         const [animal4, setAnimal4] = useState(null);
...
const columns = [
  { key: "id", label: "ID" },
  { key: "author", label: "Author" }, // Includes Image, Name, Email
  { key: "review", label: "Review" },
  { key: "rating", label: "Rating" },
  { key: "product", label: "Product" }, // Includes Product Image and Name
  ...
  { key: "type", label: "Type" }, // Reviews or Replies
  { key: "authorized", label: "Authorized" }, // Approved, Pending, Spam
  { key: "visibility", label: "Visibility" },
  { key: "status", label: "Status" }, // Active, Inactive
  { key: "action", label: "Action" }, // Edit, Delete, View Buttons
];
...
const [data,setData] =useState( [
  {
    ...
    author: {
      name: "Karen Russel",
      email: "karen.russel@example.com",
      avatar: "https://isomorphic-furyroad.s3.amazonaws.com/public/avatars/avatar-09.webp",
    },
    review:
    ...
    createDate: "20-Jan-2023",
    createTime: "10:30 AM",
    publishDate: "21-Jan-2023",
    publishTime: "11:45 AM",
  ...
  },
  {
    ...
    author: {
      name: "Irvin Ledner",
      email: "irvin.ledner@example.com",
  ...
  const fetchUniqueEmailSuggestions = (value) => {
    return [
      ...new Set(
        data
          .filter((item) =>
            item.author.email.toLowerCase().includes(value.toLowerCase())
          )
          .map((item) => item.author.email)
      ),
    ];
  };
...
const [productValue, setProductValue] = useState("");
  ...
  // Helper function to get unique product suggestions
  const fetchUniqueProductSuggestions = (value) => {
    return [
      ...new Set(
        data
          .filter((item) =>
  ...
  const handleProductInputChange = (e) => {
    ...
    if (value) {
      setProductSuggestions(fetchUniqueProductSuggestions(value));
    } else {
...
const [selectedPublisher, setSelectedPublisher] = useState(null);
  ...
  const applyFilters = () => {
    ...
    const filtered = data.filter((item) => {
      ...
      const matchesSearchTerm = searchTerm ? item.author.name.toLowerCase().includes(searchTerm.toLowerCase()) || item.author.email.toLowerCase().includes(searchTerm.toLowerCase()) || item.review.toLowerCase().includes(searchTerm.toLowerCase()) || item.product.name.toLowerCase().includes(searchTerm.toLowerCase()) || item.product.category.toLowerCase().includes(searchTerm.toLowerCase()) || item.createDate.toLowerCase().includes(searchTerm.toLowerCase()) || item.createTime.toLowerCase().includes(searchTerm.toLowerCase()) || item.publishDate.toLowerCase().includes(searchTerm.toLowerCase())  || item.publishTime.toLowerCase().includes(searchTerm.toLowerCase()) || item.updateby.toLowerCase().includes(searchTerm.toLowerCase()) || item.updateDate.toLowerCase().includes(searchTerm.toLowerCase()) || item.update_time.toLowerCase().includes(searchTerm.toLowerCase()) || item.publishBy.toLowerCase().includes(searchTerm.toLowerCase()) || item.type.toLowerCase().includes(searchTerm.toLowerCase())  || item.authorized.toLowerCase().includes(searchTerm.toLowerCase()) || item.status.toLowerCase().includes(searchTerm.toLowerCase()) : true;
      // Create Date filter
 ...
 <section className='w-[100%] m-auto py-[20px]  xl:py-[40px] px-[20px] lg:px-[30px]'>
   ...
   <div className='w-full   md:w-auto flex gap-[10px] md:m-0 mt-[15px]'>
            ...
            {/* <NavLink to="/business-new-review"className="w-[50%] md:w-auto ">
...
<div className="relative  sm:rounded-lg">
  ...
  <div className="relative w-full md:w-auto">
      ...
      <input
        ...
        placeholder="Search for product review"
        value={searchTerm}
...
<section className="p-[20px] relative w-[100%] ">
    ...
    <div className="mb-[10px] mt-[10px] relative">
      <label htmlFor="product" className="text-label_size 2xl:text-[16px] font-label_weight 2xl:font-[500] text-neutral-600">
        Product
      </label>
      <br />
      <input
                      ...
                      {options3.map((option) => (
                        ...
                        >
                          ...
                          {option.label}
                            ...
                            {
                                                                                      authorinput=="" && emailinput=="" &&  productinput=="" && updateByInput==""&&  publishByInput=="" && selectedOption=="Select Authorized" && selectedOption5=="Select Type"  && selectedOption2=="Select Visibility" && selectedOption3=="Select Status" ? "":            <button
    ...
    <div className="w-full">
      ...
      {columns.map((col) => (
                      <label
                        ...
                        className="flex p-3 w-full capitalize cursor-pointer bg-white border border-gray-300 rounded-md text-sm"
                      >
                        <input
              ...
              {columns
                ...
                .map((col) => (
                  ...
                  >
                    {/* Author Section */}
                    {col.key === "author" ? (
                      <div className="flex items-center gap-[15px] pr-[60px]">
                        <img
                        ...
                        <div className="flex flex-col gap-[5px]">
                          {
                            row.author.name.length > 20 ?           <p className="text-[14px] 2xl:text-[16px] font-[500] text-black truncate">
                            {row.author.name.slice(0,20)}..
                          </p>:          <p className="text-[14px] 2xl:text-[16px] font-[500] text-black truncate">
                            {row.author.name}
                          </p>
                          }
                      {
                            row.author.email.length > 30 ?        <p className="text-[12px] font-[500] text-brand_color">
                            {row.author.email.slice(0,30)}..
                          </p>:        <p className="text-[12px] font-[500] text-brand_color">
                            {row.author.email}
                          </p>
                          }
                     ...
                     <div>
                      {row.review.length > 40 ? <p>{row.review.slice(0,40)}..</p>:<p>{row.review}</p> }
                     </div>
        
                    ) : col.key === "product" ? (
                      <div className="flex items-center gap-[15px] pr-[40px]">
                        <img
                        ...
                        />
                        <div>
                           {row.product.name.length > 35 ?  <p className="text-[14px] font-[500] text-black truncate">
                            {row.product.name.slice(0,35)}..
                          </p>:    <p className="text-[14px] font-[500] text-black truncate">
                            {row.product.name}
                          </p>}
                    ...
                    >
                       ...
                       <div className="flex justify-start items-center gap-[12px] relative">
                                           ...
                                           <span className="absolute left-1/2 transform -translate-x-1/2 bottom-[-5px] w-0 h-0 border-x-[5px] border-x-transparent border-t-[5px] border-brand_color"></span>
                                                         ...
                                                         <NavLink to="/products/edit-product-review">
                                                         <div className="w-[30px] h-[30px]  border border-gray-300 rounded-[5px] hover:text-brand_color flex justify-center items-center text-[15px] cursor-pointer hover:border-brand_color dark:border-gray-700 dark:hover:bg-gray-800 group relative">
                                         <CiEdit />
                                         <span className="absolute hidden group-hover:block bottom-[35px] left-1/2 transform -translate-x-1/2 bg-brand_color text-white text-xs font-medium rounded px-2 py-0.5 z-10">
                                           Edit
