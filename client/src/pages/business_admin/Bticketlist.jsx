...
const [selectedPublisher, setSelectedPublisher] = useState(null);
  ...
  const applyFilters = () => {
    ...
    const filtered = data.filter((item) => {
      const matchesname = nameinput ? item.nick_name.toLowerCase().includes(nameinput.toLowerCase()) : true;
      ...
      const matchesSearchTerm = searchTerm ?  item.nick_name.toLowerCase().includes(searchTerm.toLowerCase()) || item.title.toLowerCase().includes(searchTerm.toLowerCase()) || item.message.toLowerCase().includes(searchTerm.toLowerCase()) || item.create_date.toLowerCase().includes(searchTerm.toLowerCase()) ||  item.updateDate.toLowerCase().includes(searchTerm.toLowerCase()) || item.updated_time.toLowerCase().includes(searchTerm.toLowerCase()) || item.status.toLowerCase().includes(searchTerm.toLowerCase()) : true;
  ...
  useEffect(() => {
    applyFilters();
  }, [searchTerm,nameinput, publishByInput,createbyinput,updateByInput, authorized, visibility,status]);
 ...
 <section className='w-[100%] m-auto py-[20px]  xl:py-[40px] px-[20px] lg:px-[30px]'>
     <div className='w-full flex md:justify-between items-center  md:flex-row flex-col justify-start'>
      ...
      <div className='w-full   md:w-auto flex gap-[10px] md:m-0 mt-[15px]'>
            ...
            <NavLink to="/support-ticket/new-ticket"className="w-[50%] md:w-auto ">
        ...
        {options.map((option) => (
          ...
          >
            ...
            {option.label}
                ...
                {
                                      nameinput=="" &&  createbyinput=="" &&  publishByInput=="" &&  updateByInput=="" && selectedOption=="Select Status" ? "":            <button
                  ...
                  >
                    {col.key === "name" ? (
                      ...
                      <div>
                        {/* Display Title */}
                       {
                        row.title.length > 30 ?  <h1 className="text-[15px] text-nowrap 2xl:text-[16px] font-[500] text-black dark:text-gray-100">{row.title.slice(0,30)}...</h1>: <h1 className="text-[15px] text-nowrap 2xl:text-[16px] font-[500] text-black dark:text-gray-100">{row.title}</h1>
                       }
                        {/* Display Message */}
                        ...
                        <div className="text-[14px] text-gray-400">
                          {row.updated_time}
                             ...
                             <div className="flex justify-start items-center gap-[12px] relative">
                                                   ...
                                                   <NavLink to="/support-ticket/view-ticket" className="w-[30px] h-[30px]  border border-gray-300 rounded-[5px] hover:text-brand_color flex justify-center items-center text-[15px] cursor-pointer hover:border-brand_color dark:border-gray-700 dark:hover:bg-gray-800 group relative">
                                                     <GoEye  />
                                     ...
                                     <span className="absolute left-1/2 transform -translate-x-1/2 bottom-[-5px] w-0 h-0 border-x-[5px] border-x-transparent border-t-[5px] border-brand_color"></span>
                                                   ...
                                                   <NavLink to="/support-ticket/replay-ticket" className="w-[30px] h-[30px]  border border-gray-300 rounded-[5px] hover:text-brand_color flex justify-center items-center text-[15px] cursor-pointer hover:border-brand_color dark:border-gray-700 dark:hover:bg-gray-800 group relative">
                                                     <MdOutlineMail />
                                                     <span className="absolute hidden group-hover:block bottom-[35px] left-1/2 transform -translate-x-1/2 bg-brand_color text-white text-xs font-medium rounded px-2 py-0.5 z-10">
                                                       Replay
                                                         <span className="absolute left-1/2 transform -translate-x-1/2 bottom-[-5px] w-0 h-0 border-x-[5px] border-x-transparent border-t-[5px] border-brand_color"></span>
