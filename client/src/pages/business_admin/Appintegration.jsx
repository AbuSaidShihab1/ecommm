...
const [data,setData] = useState([
  {
    id: "1",
    image:
      "https://isomorphic-furyroad.vercel.app/_next/image?url=https%3A%2F%2Fisomorphic-furyroad.s3.amazonaws.com%2Fpublic%2Favatars%2Favatar-02.webp&w=1920&q=75",
    name: "Wordpress",
    description: "Lorem ipsum dolor sit amet consectetur adipisicing elit.",
    version: "1.0.0",
    update_date: "21-October-2023",
    update_time:"04:10 PM",
    status: "Inactive",
    action: "",
  },
  {
...
const columns = [
  ...
  const handleGenerate = () => {
    const generatedApi = `https://api.example.com/resource/${Math.random().toString(36).substring(7)}`;
    setApiValue(generatedApi);
  };
...
const [selectedPublisher, setSelectedPublisher] = useState(null);
  ...
  const applyFilters = () => {
    const filtered = data.filter((item) => {
      ...
      const matchesSearchTerm = searchTerm ?  item.name.toLowerCase().includes(searchTerm.toLowerCase()) || item.description.toLowerCase().includes(searchTerm.toLowerCase()) || item.update_date.toLowerCase().includes(searchTerm.toLowerCase()) || item.update_time.toLowerCase().includes(searchTerm.toLowerCase()) ||  item.status.toLowerCase().includes(searchTerm.toLowerCase()) : true;
  ...
  useEffect(() => {
    applyFilters();
  }, [searchTerm, publishByInput, updateByInput, status]);
 ...
 <section className='w-[100%] m-auto py-[20px]  xl:py-[40px] px-[20px] lg:px-[30px]'>
     <div className='w-full flex md:justify-between items-center  md:flex-row flex-col justify-start'>
        <div className='w-full md:w-auto'>
              <h1 className='text-[20px] font-[600] mb-[8px]'>App Integration</h1>
       ...
       <div className='w-full   md:w-auto flex gap-[10px] md:m-0 mt-[15px]'>
        ...
        {/* <NavLink to="/business-new-post"className="w-[50%] md:w-auto ">
           <button className='px-[22px] w-[100%] md:w-auto py-[10px] border-[2px] border-brand_color  text-white rounded-[5px] text-[14px] bg-brand_color flex justify-center items-center gap-[10px]'>
   ...
   <form action="">
  <label htmlFor="apiInput" className="text-[20px] text-neutral-700">Rest API</label> <br />
  <div className="relative mt-[8px]">
    <input
      ...
      className="block w-full px-4 py-3 text-sm outline-none text-gray-900 border border-gray-300 rounded-[2px] bg-white focus:ring-brand_color focus:border-brand_color dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-brand_color dark:focus:border-brand_color"
      placeholder="Generate your API"
      required
    />
    <div className="absolute right-0 bottom-0 flex h-[100%]">
      <button
        type="button"
        onClick={handleGenerate}
        className="text-white bg-brand_color border-r-[1px] border-orange-600 hover:bg-brand_color  font-medium text-sm px-4 py-2"
      >
        Generate
      </button>
      <button
        type="button"
        onClick={handleCopy}
        className="text-white bg-brand_color hover:bg-brand_color  font-medium rounded-r-[2px] text-sm px-4 py-2"
      >
        <MdContentCopy className='text-[25px]'/>
      </button>
    </div>
  </div>
</form>
...
<div className="relative  sm:rounded-lg">
  ...
  <div className="relative w-full md:w-auto">
      ...
      <input
        ...
        placeholder="Search for app integration"
        value={searchTerm}
        ...
        {options.map((option) => (
          ...
          >
            ...
            {option.label}
               ...
               {
                                       selectedOption=="Select Status" ? "":<button
                  ...
                  .map((col) => (
                    <td
                      ...
                      className={`px-4 py-4 text-sm text-nowrap  ${
                        col.key === "description" ? " text-gray-800 text-[17px] dark:text-gray-100" : "text-[17px] text-gray-500 dark:text-gray-300"
                      }`}
                    >
                      {col.key === "image" ? (
                        <div className='flex justify-start items-center gap-[10px] pr-[50px]'>
                               <img
                          src={row[col.key]}
                          alt="Row"
                        ...
                        />
                          ...
                          <span className='text-black text-[16px] font-[500]'>{row.name}</span>
                        </div>
                        </div>
                        ...
                        <div className="flex items-center text-nowrap">
                          {
                            row.description.length > 30 ? <span className=''>{row.description.slice(0,30)}...</span>:<span className=''>{row.description}</span>
                          }
                        </div>
                      ):col.key === "status" ? (
                        ...
                        <div className="flex justify-start pl-[25px] items-center gap-[5px]">
