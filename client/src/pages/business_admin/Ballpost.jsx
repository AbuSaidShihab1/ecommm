...
const [data,setData] = useState([
  {
    id: 3,
    Image:
      "https://isomorphic-furyroad.vercel.app/_next/image?url=https%3A%2F%2Fisomorphic-furyroad.s3.amazonaws.com%2Fpublic%2Favatars%2Favatar-10.webp&w=1920&q=75",
    title: "Exploring the Evolution of Gastronomy: A Historical Perspective.",
    categories: "Food",
    comment: "20",
    views: 200,
  ...
  },
  {
    ...
    title: "Mastering the Art of Flavor Pairing in Modern Cuisine",
    categories: "Food",
    comment: "20",
    views: 200,
  ...
  },
  {
    ...
    author: "Zobaer Ahmmed",
    create_date: "21-October-2023",
    create_time: "10:45 AM",
    publish_by: "Zobaer Ahmmed",
    publish_date: "21-October-2023",
    publish_time: "11:15 AM",
           updateby: "Zobaer Ahmmed",
    updateDate: "21-October-2023",
    update_time: "08:20 PM",
    authorized: "Pending",
  ...
  },
  {
    ...
    Image:
      "https://isomorphic-furyroad.vercel.app/_next/image?url=https%3A%2F%2Fisomorphic-furyroad.s3.amazonaws.com%2Fpublic%2Favatars%2Favatar-10.webp&w=1920&q=75",
    title: "The whole world depends on technology",
...
const columns = [
 ...
 const [visibleColumns, setVisibleColumns] = useState(
  ...
  const [categoruinput,setcategoryinput]=useState("")
  ...
  const handleCategoryChange = (e) => {
    ...
    // Filter unique categories based on user input
  ...
  const handleCategorySelect = (category) => {
    setcategoryinput(category); // Set the selected category in the input
  ...
  const handleAuthorChange = (e) => {
    ...
    // Filter unique authors based on user input
  ...
  const handleAuthorSelect = (author) => {
    setauthorinput(author); // Set the selected author in the input
    setFilteredAuthors([]); // Clear suggestions
  };
  // ------------------publish by----------------
  const [searchPublishBy, setSearchPublishBy] = useState("");
  const [filteredPublishBy, setFilteredPublishBy] = useState([]);

  // Extract unique publish_by names
  const uniquePublishBy = [...new Set(data.map((item) => item.publish_by))];
  ...
  const handlePublishByChange = (e) => {
    ...
    if (value.trim() === "") {
      setFilteredPublishBy([]);
      return;
    }

    // Filter unique publish_by names based on user input
    const filtered = uniquePublishBy.filter((publishBy) =>
      publishBy.toLowerCase().includes(value.toLowerCase())
    );
    setFilteredPublishBy(filtered);
  };

  // Handle suggestion click
  const handlePublishBySelect = (publishBy) => {
    console.log(publishBy)
    setPublishByInput(publishBy); // Set the selected publish_by in the input
    setFilteredPublishBy([]); // Clear suggestions
  };

  // ----------------------update by----------------
  const [searchUpdateBy, setSearchUpdateBy] = useState("");
  const [filteredUpdateBy, setFilteredUpdateBy] = useState([]);
    // Extract unique update_by names
    ...
    const handleUpdateByChange = (e) => {
      ...
      // Filter unique update_by names based on user input
      const filtered = uniqueUpdateBy.filter((updateBy) => 
        updateBy && updateBy.toLowerCase().includes(value.toLowerCase()) // Check if updateBy is defined
      );
      setFilteredUpdateBy(filtered);
    };
...
const [selectedPublisher, setSelectedPublisher] = useState(null);
  ...
  const applyFilters = () => {
    const filtered = data.filter((item) => {
      const matchedCategory = categoruinput ? item.categories.toLowerCase().includes(categoruinput.toLowerCase()) : true;
      ...
      const matchesSearchTerm = searchTerm ? item.title.toLowerCase().includes(searchTerm.toLowerCase()) || item.author.toLowerCase().includes(searchTerm.toLowerCase()) || item.categories.toLowerCase().includes(searchTerm.toLowerCase()) || item.comment.toLowerCase().includes(searchTerm.toLowerCase()) || item.create_date.toLowerCase().includes(searchTerm.toLowerCase()) || item.create_time.toLowerCase().includes(searchTerm.toLowerCase()) || item.publish_by.toLowerCase().includes(searchTerm.toLowerCase()) || item.publish_date.toLowerCase().includes(searchTerm.toLowerCase()) || item.publish_time.toLowerCase().includes(searchTerm.toLowerCase()) || item.updateby.toLowerCase().includes(searchTerm.toLowerCase()) || item.updateDate.toLowerCase().includes(searchTerm.toLowerCase()) || item.update_time.toLowerCase().includes(searchTerm.toLowerCase()) || item.authorized.toLowerCase().includes(searchTerm.toLowerCase()) || item.visibility.toLowerCase().includes(searchTerm.toLowerCase()) || item.status.toLowerCase().includes(searchTerm.toLowerCase()) : true;
  ...
  useEffect(() => {
    applyFilters();
  }, [searchTerm, categoruinput, authorinput,publishByInput, updateByInput, authorized, visibility, status, createRange, publishRange]);
 ...
 <section className='w-[100%] m-auto py-[20px]  xl:py-[40px] px-[20px] lg:px-[30px]'>
     <div className='w-full flex md:justify-between items-center  md:flex-row flex-col justify-start'>
        <div className='w-full md:w-auto'>
              <h1 className='text-[20px] font-[600] mb-[8px]'>Post List</h1>
        ...
        {/* ---------------table --------------- */}
         <div className='w-full   md:w-auto flex gap-[10px] md:m-0 mt-[15px]'>
               ...
               <NavLink to="/contents/new-post-content"className="w-[50%] md:w-auto ">
...
<div className="relative  sm:rounded-lg">
    <div className="flex items-center justify-between mb-[14px] flex-column md:flex-row flex-wrap space-y-4 md:space-y-0 py-4 bg-white dark:bg-gray-900">
 {/* -------------------search box----------------------- */}
...
<section className="p-[20px] relative w-[100%] ">
    <div className="w-full ">
      ...
      {filteredCategories.length > 0 && (
        <ul className="absolute z-10 bg-white border border-[#ddd] rounded mt-[5px] max-h-[150px] overflow-y-auto w-full">
    ...
    {showCreateCalendar && (
      <div className="absolute z-10 bg-white shadow-md border-[1px] border-[#eee] w-full p-2">
        ...
        <div className="flex justify-end gap-2 mt-2">
          <button onClick={cancelCreateDate} className="px-3 py-1 border text-[13px] 2xl:text-[15px] rounded bg-gray-300">Cancel</button>
          <button onClick={applyCreateDate} className="px-3 py-1 border text-[13px] 2xl:text-[15px] rounded bg-brand_color text-white">Apply</button>
        </div>
      </div>
    )}
  </div>
  ...
  <div style={{ position: "relative", width: "100%" }}>
    ...
    {showPublishCalendar && (
      ...
      <input
        ...
        className="w-full mt-[3px] 2xl:mt-[7px] rounded-[5px] placeholder-gray-700 outline-brand_color text-input_text 2xl:text-[15px] h-input_height 2xl:h-[45px] border-[1px] border-[#eee] p-[12px]"
      />
      {filteredPublishBy.length > 0 && (
        <ul className="absolute z-10 bg-white border border-[#ddd] rounded mt-[5px] max-h-[150px] overflow-y-auto w-full">
          {filteredPublishBy.map((publishBy, index) => (
            <li
              key={index}
              className="p-[10px] hover:bg-gray-100 cursor-pointer"
              onClick={() => handlePublishBySelect(publishBy)}
            >
              {publishBy}
            </li>
          ))}
  ...
  <div style={{ position: "relative", width: "100%" }}>
    ...
    {showUpdateCalendar && (
      <div className="absolute z-10 bg-white shadow-md border-[1px] border-[#eee] w-full p-2">
        ...
        <div className="flex justify-end gap-2 mt-2">
          <button onClick={cancelUpdateDate} className="px-3 py-1 border rounded text-[13px] 2xl:text-[15px] bg-gray-300">Cancel</button>
      ...
      {filteredUpdateBy.length > 0 && (
        <ul className="absolute z-10 bg-white border border-[#ddd] rounded mt-[5px] max-h-[150px] overflow-y-auto w-full">
          {filteredUpdateBy.map((updateBy, index) => (
            <li
              key={index}
              className="p-[10px] hover:bg-gray-100 cursor-pointer"
              onClick={() => handleUpdateBySelect(updateBy)}
            >
              {updateBy}
            </li>
          ))}
              ...
              {options3.map((option) => (
                ...
                >
                  ...
                  {option.label}
                         ...
                         {
                                      categoruinput=="" && authorinput=="" &&  publishByInput=="" && updateByInput=="" && selectedOption=="Select Authorized" && selectedOption2=="Select Visibility" && selectedOption3=="Select Status" ? "":            <button
                    ...
                    {col.key === "Image" ? (
                     <div className="flex justify-start items-center pr-[50px] gap-[15px]  lg:pr-0">
                              ...
                              <div className='pr-[50px] '>
                                {row.title.length > 30 ? <h1 className="text-[12px] text-nowrap lg:text-[16px] font-[500] text-black">{row.title.slice(0,30)}..</h1>:<h1 className="text-[12px] lg:text-[16px] text-nowrap font-[500] text-black">{row.title}</h1>}
                                <p className="text-[8px] lg:text-[13px] mt-[4px] font-[500] text-neutral-600">{row.categories}</p>
                              </div>
                            </div>
                    ): col.key === "status" ? (
                      ...
                      <div>
                        ...
                        <div className="text-[14px] text-gray-400">{row.create_time}</div>
                      </div>
                    ) :col.key === "updateDate" ? (
                      <div>
                        <div className="text-black dark:text-gray-100">
                      ...
                      <div>
                        ...
                        <div className="text-[14px] text-gray-400">{row.publish_time}</div>
                      </div>
                    ) : col.key === "authorized" ? (
                      <span
