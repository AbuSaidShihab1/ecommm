...
const Newpriceplan = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (validate()) {
      setIsLoading(true);
      try {
        const response = await axios.post(`${base_url}/super/admin/new-price-plan`, {
        ...
        if (response.data.success) {
          setToastMessage('Price plan created successfully!');
          setToastType('success');
        }
      } catch (error) {
        console.error('Error submitting form:', error);
        setToastMessage(error.response?.data?.message || 'Failed to create price plan');
        setToastType('error');
      } finally {
        setIsLoading(false);
      }
      ...
      <section className={activesidebar ? 'w-[100%] h-[100vh] overflow-y-auto transition-all duration-300' : ' transition-all duration-300 w-[100%] overflow-y-auto xl:w-[85%] h-[100vh]'}>
        ...
        <section className='w-[100%] m-auto py-[20px] xl:py-[40px] px-[20px] lg:px-[30px]'>
          <div className='w-full flex justify-between items-center'>
            <div>
              <h1 className='text-[20px] lg:text-[20px] font-[600] mb-[8px]'>New Price Plan</h1>
                                                ...
                                                <div key={image.id}>
                                                    <img
                                                      src={`http://localhost:8080/uploads/${admin_info?._id}/${image}`}
                                                        alt={image.title}
                                                        className="border rounded w-[100px] h-[100px] cursor-pointer "
    ...
    <div className="w-[60px] h-[60px] relative">
      ...
      <div className="absolute inset-2 rounded-full border-[4px] border-t-transparent border-brand_color animate-spin-reverse"></div>
              ...
              <div className='flex justify-end items-center gap-[10px]'>
                ...
                <button type="submit" className='px-[30px] py-[10px] text-white text-[14px] gap-[8px] bg-brand_color flex justify-center items-center rounded-[5px] cursor-pointer'>Submit</button>
              </div>
            </form>
          </section>
        </section>
