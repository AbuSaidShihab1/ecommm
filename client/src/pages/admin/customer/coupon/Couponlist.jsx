...
const Couponlist = () => {
   const navigate=useNavigate();
     ...
     const { coupons, loadingCoupons,fetchCoupons} = useSuper();
...
//     start_time: "09:00 AM",
//     expirydate: "15-February-2024",
//     expire_time: "11:59 PM",
//     createdate: "15-January-2024",
//     create_time: "11:00 AM",
//     createby: "Zobaer Ahmmed",
//     publishdate: "16-January-2024",
//     publish_time: "10:30 AM",
//     publishby: "Zobaer Ahmmed",
//     updatedate: "20-January-2024",
//     update_time: "01:15 PM",
//     updateby: "Zobaer Ahmmed",
//     authorized: "Pending",
//     visibility: "Draft",
//     status: "Inactive",
...
//     expire_time: "11:59 PM",
//     createdate: "20-January-2024",
//     create_time: "09:30 AM",
//     createby: "Abu Said Shihab",
//     publishdate: "21-January-2024",
//     publish_time: "12:00 PM",
//     publishby: "Zobaer Ahmmed",
//     updatedate: "25-January-2024",
...
//     createdate: "01-January-2024",
//     create_time: "10:00 AM",
//     createby: "Rifat Ahmmed",
//     publishdate: "02-January-2024",
//     publish_time: "09:00 AM",
//     publishby: "Said Ahmmed",
//     updatedate: "10-January-2024",
//     update_time: "02:30 PM",
//     updateby: "Shihab Hasan",
//     authorized: "Approved",
//     visibility: "Publish",
//     status: "Active",
//     action: "",
...
const columns = [
  ...
  { key: "couponAmount", label: "Amount" },
  { key: "usageLimitPer", label: "Usage Limit" },
                         ...
                         const [selectedOption5, setSelectedOption5] = useState('Select Type');
                                          ...
                                          const [hoveredItem, setHoveredItem] = useState(null); // Tracks the currently hovered item
                                           ...
                                           const options5 = [
                                             ...
                                             { label: 'Fixed', color: 'text-green-500', bgColor: 'bg-green-500' },
                                             { label: 'Percentage', color: 'text-orange-500', bgColor: 'bg-orange-500' },
                                           ];
...
const [expiryRange, setExpiryRange] = useState([{ startDate: new Date(), endDate: new Date(), key: "selection" }]);
const [tempExpiryRange, setTempExpiryRange] = useState(expiryRange);
// Functions for Expiry Date
const handleExpiryDateChange = (item) => setTempExpiryRange([item.selection]);
const applyExpiryDate = () => { 
  setExpiryRange(tempExpiryRange); 
  setShowExpiryCalendar(false); 
};
const cancelExpiryDate = () => { 
  setTempExpiryRange(expiryRange); 
  setShowExpiryCalendar(false); 
};
// ---------------authorized-by---------------------
...
const [selectedPublisher, setSelectedPublisher] = useState(null);
  ...
  const handleSearchChange = (e) => {
    ...
    setFilteredPublishers(filtered.map((name) => ({ id: name, authorizedBy: name })));
  };
  ...
  const applyFilters = () => {
    ...
    const filtered = data.filter((item) => {
      const matchesauthor = authorinput ? item.author.name.toLowerCase().includes(authorinput.toLowerCase()) : true;
      ...
      const matchestype = type ? item.discountType === type : true;
      const matchesVisibility = visibility ? item.visibility === visibility : true;
      const matchesStatus = status ? item.status === status : true;
      const matchesSearchTerm = searchTerm ? item.couponCode.toLowerCase().includes(searchTerm.toLowerCase()) || item.description.toLowerCase().includes(searchTerm.toLowerCase()) || item.discountType.toLowerCase().includes(searchTerm.toLowerCase()) || item.startDate.toLowerCase().includes(searchTerm.toLowerCase()) ||  item.expiryDate.toLowerCase().includes(searchTerm.toLowerCase()) || item.createDate.toLowerCase().includes(searchTerm.toLowerCase())  ||  item.updateBy.toLowerCase().includes(searchTerm.toLowerCase()) || item.createBy.toLowerCase().includes(searchTerm.toLowerCase()) ||  item.authorizedDate.toLowerCase().includes(searchTerm.toLowerCase()) || item.authorized.toLowerCase().includes(searchTerm.toLowerCase())   || item.status.toLowerCase().includes(searchTerm.toLowerCase()) : true;
      // Create Date filter
  ...
  useEffect(() => {
    applyFilters();
  }, [searchTerm,authorinput,emailinput,productinput,createByInput,publishByInput, updateByInput, authorized,type,visibility, status, createRange, publishRange]);
...
const handleDeleteTicket = async () => {
  ...
  try {
    const response = await fetch(`${base_url}/super/admin/coupons/${ticketToDelete}`, {
      method: 'DELETE',
      headers: {
    ...
    if (data) {
      ...
      toast.error(data.message || 'Failed to delete Coupons', {
        id: toastId,
        duration: 4000,
      });
    }
 ...
 <section className='w-[100%] m-auto py-[20px]  xl:py-[40px] px-[20px] lg:px-[30px]'>
     <div className='w-full flex md:justify-between items-center  md:flex-row flex-col justify-start'>
        <div className='w-full md:w-auto'>
              <h1 className='text-[20px] font-[600] mb-[8px]'>Coupon List</h1>
       ...
       <div className='w-full   md:w-auto flex gap-[10px] md:m-0 mt-[15px]'>
        ...
        <NavLink to="/super-new-coupon"className="w-[50%] md:w-auto ">
  ...
  <div className="relative w-full md:w-auto">
   ...
   <div className='w-full md:w-auto flex justify-center items-center gap-[10px]'>
    <button onClick={()=>{setfilter_sidebar(true)}}  className='w-[50%] md:w-auto text-[14px] border-[1px] border-[#eee] hover:text-brand_color transition-all duration-100 px-[17px] py-[7px] flex justify-center hover:border-brand_color items-center gap-[7px] rounded-[5px]'>
          <BiFilterAlt className='text-[22px]'/> Filters
    </button>
...
<section className="p-[20px] relative w-[100%] ">
  ...
  <label className="text-label_size 2xl:text-[16px] text-neutral-600 font-label_weight">Start Date</label>
  <br />
...
<div className="mb-[10px]">
  <label className="text-label_size 2xl:text-[16px] text-neutral-600 font-label_weight">Expiry Date</label>
  <br />
  <div style={{ position: "relative", width: "100%" }}>
    <input
      type="text"
      readOnly
      value={`${format(expiryRange[0].startDate, "dd-MM-yyyy")} - ${format(expiryRange[0].endDate, "dd-MM-yyyy")}`}
    ...
    {showExpiryCalendar && (
      <div className="absolute z-10 bg-white shadow-md border-[1px] border-[#eee] w-full p-2">
        <DateRange
          ranges={tempExpiryRange}
          onChange={handleExpiryDateChange}
          months={1}
          rangeColors={["#FF6A00"]}  // Custom color for expiry date range
        />
        <div className="flex justify-end gap-2 mt-2">
          <button onClick={cancelExpiryDate} className="px-3 py-1 border rounded bg-gray-300">Cancel</button>
          <button onClick={applyExpiryDate} className="px-3 py-1 border rounded bg-indigo-600 text-white">Apply</button>
        </div>
      </div>
    )}
  </div>
  ...
  <div style={{ position: "relative", width: "100%" }}>
    ...
    {showCreateCalendar && (
      ...
      <label htmlFor="createBy" className="text-label_size 2xl:text-[16px] text-neutral-600 font-label_weight">
        Create By
      </label>
      <br />
      <input
                    ...
                    <ul className="absolute mt-1 w-full bg-white border border-gray-200 rounded-lg shadow-lg z-50 max-h-60 overflow-auto">
                      {options3.map((option) => (
                        ...
                        >
                         ...
                         {
                                                                                                          type==""&&  updateByInput==""&& createByInput=="" && publishByInput=="" &&  selectedOption=="Select Authorized" && selectedOption2=="Select Visibility" && selectedOption3=="Select Status" ? "":            <button
         ...
         <tr key={rowIndex} className="">
              {columns
                ...
                .map((col) => (
                  <td
                    key={col.key}
                    className={`lg:px-4 py-4 text-sm w-full lg:w-[45%] px-[40px] whitespace-nowrap ${
                      col.key === "status"
                  ...
                  >
                    {col.key === "image" ? (
                      ...
                      <div>
                        <div className="whitespace-nowrap font-[500] text-black dark:text-gray-100">
                          {row[col.key]}
                        </div>
                      </div>
                    ): col.key === "authorized" ? (
                      ...
                      <div className="text-[14px] text-gray-400 text-nowrap">
                        {format(new Date(row[col.key]), 'hh:mm a')}
                         ...
                         <div className="flex justify-start items-center gap-[12px] relative">
                                                        ...
                                                        <NavLink to={`/super-view-coupon/${row._id}`}  className="w-[30px] h-[30px]  border border-gray-300 rounded-[5px] hover:text-brand_color flex justify-center items-center text-[15px] cursor-pointer hover:border-brand_color dark:border-gray-700 dark:hover:bg-gray-800 group relative">
                                                          <GoEye  />
                                          ...
                                          <span className="absolute left-1/2 transform -translate-x-1/2 bottom-[-5px] w-0 h-0 border-x-[5px] border-x-transparent border-t-[5px] border-brand_color"></span>
                                                        ...
                                                        <NavLink to={`/super-edit-coupon/${row._id}`} className="w-[30px] h-[30px]  border border-gray-300 rounded-[5px] hover:text-brand_color flex justify-center items-center text-[15px] cursor-pointer hover:border-brand_color dark:border-gray-700 dark:hover:bg-gray-800 group relative">
                                                          <CiEdit />
                                                          <span className="absolute hidden group-hover:block bottom-[35px] left-1/2 transform -translate-x-1/2 bg-brand_color text-white text-xs font-medium rounded px-2 py-0.5 z-10">
                                                            Edit
                                                              <span className="absolute left-1/2 transform -translate-x-1/2 bottom-[-5px] w-0 h-0 border-x-[5px] border-x-transparent border-t-[5px] border-brand_color"></span>
