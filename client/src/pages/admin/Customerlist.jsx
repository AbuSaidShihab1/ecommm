...
const Priceaccess = ({ isOpen, onClose }) => {
  const plans = ["Free Plan", "Starter Plan", "Advance Plan", "Premium Plan"];
  ...
  return isOpen ? (
    <div className="fixed inset-0 w-full bg-black bg-opacity-50 h-[100vh] overflow-y-auto py-[40px] z-[100000000] flex items-center justify-center">
      <section className="w-[90%] lg:w-[70%] xl:w-[60%] 2xl:w-[50%] rounded-[3px] bg-white m-auto">
        ...
        <div className="border-b w-full px-5 py-4 flex justify-between items-center border-gray-200">
          <h1 className="text-[15px] 2xl:text-[18px] font-[500] text-gray-600">Plan Access</h1>
        ...
        <div className='px-6 pt-5'>
          <div className="relative w-[100%] ">
            ...
            <input
              ...
              disabled={customPlan}
              className="w-full mt-1 rounded-md border p-2 text-gray-700 placeholder-gray-500 outline-brand_color"
            />
            {showSuggestions && (
        ...
        <div className="px-6 py-4">
          <div className="flex items-center gap-2">
            ...
            <p className='text-label_size 2xl:text-[16px] font-label_weight 2xl:font-[500] text-gray-600'>Custom Plan Access</p>
          </div>
          {/* --------------plan-module------------------ */}
          {
    ...
    <div className="fixed inset-0 w-full bg-black bg-opacity-50 h-[100vh] overflow-y-auto py-[40px] z-[100000000] flex items-center justify-center">
      <section className="w-[90%] lg:w-[70%] xl:w-[60%] 2xl:w-[50%] rounded-[3px] bg-white m-auto ">
        ...
        <div className="border-b px-5 py-4 flex justify-between items-center border-gray-200">
          <h1 className="text-[15px] 2xl:text-[18px] font-[500] text-gray-600">Category Access</h1>
          <IoClose className="text-2xl cursor-pointer text-gray-600 hover:text-gray-800" onClick={onClose} />
        </div>
        ...
        <div className="px-6 py-4">
          <div className="flex items-center gap-2">
            <input type="checkbox" onClick={handleCustomPlan} />
            <p className='text-label_size 2xl:text-[16px] font-label_weight 2xl:font-[500] text-gray-600'>Custom Modul Acess</p>
          </div>
          {/* --------------plan-module------------------ */}
          {
...
const Customerlist = () => {
  const navigate = useNavigate();
  const { data,setData,loadingCustomers } = useSuper();
  const { activesidebar, setactivesidebar, activetopbar, setactivetopbar } = useContext(Contextapi);
...
// },[])
  ...
  //     username: "Abu Said Shihab",
  //     phone: "+1 123 456 7890",
  //     plan: "Build Ecommerce System",
  //     category: "Ecommerce",
  ...
  //     last_active: "21-October-2023",
  //     last_active_time: "08:20 PM",
  //     authorized: "Approved",
  //     visibility: "Publish",
  //     invoice: "Deactive",
  //     status: "Active",
  ...
  //       designation: "CEO",
  //     },
  //     shop_name: "Enterprise Ltd.",
  //     username: "Rakib Hossain",
  //     phone: "+1 987 654 3210",
  //     plan: "LMS System ",
  //     category: "Ecommerce",
  //     website: "https://enterpriseltd.com",
  //     createdBy: "Rakib Hossain",
  //     createDate: "15-Jan-2023",
  //     createTime: "10:30:00",
  //     publishDate: "15-Jan-2023",
  //     publishTime: "12:00 PM",
  ...
  const columns = [
    ...
    { key: "selectedPackagename", label: "Plan" },
    { key: "category", label: "Category" },
    { key: "subDomain", label: "Website" },
  ...
  // --------------------Customer Type-----------
  const [selectedCustomerType, setSelectedCustomerType] = useState('Select Invoice');
  ...
  const customerTypeOptions = [
    { label: 'Select Invoice', color: 'text-gray-400', bgColor: 'bg-gray-300' },
    { label: 'Active', color: 'text-green-500', bgColor: 'bg-green-500' },
    { label: 'Deactive', color: 'text-red-500', bgColor: 'bg-red-500' },
  ];
  ...
  const handleCustomerTypeSelect = (option) => {
    ...
    setIsCustomerTypeOpen(false);
    if (option.label == "Select Invoice") {
  ...
  const fetchUniqueDesignationSuggestions = (value) => {
    return [
      ...new Set(
        data
          .filter((item) =>
            item.selectedPackagename.toLowerCase().includes(value.toLowerCase())
          )
          .map((item) => item.selectedPackagename)
      ),
    ];
  };
  ...
  const fetchUniqueUpdateBySuggestions = (value) => {
    return [
      ...new Set(
        data
          .filter((item) =>
            item.updatedBy.toLowerCase().includes(value.toLowerCase())
          )
  ...
  // Last Active Date States
  const [showLastActiveCalendar, setShowLastActiveCalendar] = useState(false);
  const [lastActiveRange, setLastActiveRange] = useState([{ startDate: new Date(), endDate: new Date(), key: "selection" }]);
  const [tempLastActiveRange, setTempLastActiveRange] = useState(lastActiveRange);

  // Functions for Last Active Date
  const handleLastActiveDateChange = (item) => setTempLastActiveRange([item.selection]);
  const applyLastActiveDate = () => { setLastActiveRange(tempLastActiveRange); setShowLastActiveCalendar(false); };
  const cancelLastActiveDate = () => { setTempLastActiveRange(lastActiveRange); setShowLastActiveCalendar(false); };
  ...
  const applyFilters = () => {
    ...
    const filtered = data.filter((item) => {
      const matchesdesignation = designationinput ? item.selectedPackagename.toLowerCase().includes(designationinput.toLowerCase()) : true;
      const matchescompany = companyinput ? item.category.toLowerCase().includes(companyinput.toLowerCase()) : true;
      ...
      const matchesVisibility = customertypeinput ? item.invoice === customertypeinput : true;
      const matchesstatus = status ? item.status === status : true;
      const matchessell = selltypeinput ? item.sellType === selltypeinput : true;
      const matchesVisibility2 = visibility ? item.visibility === visibility : true;

      const matchesSearchTerm = searchTerm ? item.username.toLowerCase().includes(searchTerm.toLowerCase()) || item.organizationName.toLowerCase().includes(searchTerm.toLowerCase()) || item.phone.toLowerCase().includes(searchTerm.toLowerCase()) || item.category.toLowerCase().includes(searchTerm.toLowerCase()) || item.subDomain.toLowerCase().includes(searchTerm.toLowerCase()) || item.createdBy.toLowerCase().includes(searchTerm.toLowerCase()) || item.authorizedBy.toLowerCase().includes(searchTerm.toLowerCase()) || item.updatedDate.toLowerCase().includes(searchTerm.toLowerCase()) || item.updatedBy.toLowerCase().includes(searchTerm.toLowerCase()) ||  item.authorized.toLowerCase().includes(searchTerm.toLowerCase()) || item.visibility.toLowerCase().includes(searchTerm.toLowerCase())  : true;
...
console.log("hello",data)
  ...
  const clear_filter_data = () => {
    ...
    setstateinput("");
    setcountryinput("");
    setSelectedCustomerType("Select Invoice");
    setCreateByInput("");
    setPublishByInput("");
    setUpdateByInput("");
    setSearchTerm("");
  ...
  const toggleStatus2 = (rowIndex, newStatus) => {
    console.log(rowIndex)
    const updatedRows = [...data]; // Copy the rows array
    console.log(updatedRows)
    updatedRows[rowIndex].invoice = newStatus; // Update the status of the specific row
    setData(updatedRows); // Set the updated rows to the state
  };
...
const handleToggleStatus = async (rowIndex, newStatus) => {
  const customerId = data[rowIndex]._id; // adjust based on your data structure
  try {
    await axios.patch(`${base_url}/super/admin/customers/${customerId}/status`, { status: newStatus });
    const updatedRows = [...data];
    updatedRows[rowIndex] = newStatus;
    setData(updatedRows); // assuming `rows` is your table data state
        ...
        <section className='w-[100%] m-auto py-[20px]  xl:py-[40px] px-[20px] lg:px-[30px]'>
          <div className='w-full flex md:justify-between items-center  md:flex-row flex-col justify-start'>
            <div className='w-full md:w-auto'>
              <h1 className='text-[20px] font-[600] mb-[8px]'>Customer List </h1>
            ...
            <div className='w-full   md:w-auto flex gap-[10px] md:m-0 mt-[15px]'>
              ...
              <NavLink to="/super-new-customer" className="w-[50%] md:w-auto ">
          ...
          <section className='mt-[2px] lg:mt-[20px] '>
            <div className="relative  sm:rounded-lg">
                                ...
                                <div className="w-full ">
                                  <div className=" w-full ">
                                    <div className="box rounded-xl bg-white w-full ">
                                      ...
                                      <div className="mb-[10px]">
                                        <label className="text-label_size 2xl:text-[16px] text-neutral-600 font-label_weight">Last Active Date</label>
                                        <br />
                                        <div style={{ position: "relative", width: "100%" }}>
                                          <input
                                            type="text"
                                            readOnly
                                            value={`${format(lastActiveRange[0].startDate, "dd-MM-yyyy")} - ${format(lastActiveRange[0].endDate, "dd-MM-yyyy")}`}
                                            onClick={() => setShowLastActiveCalendar(true)}
                                            placeholder="Select Date Range"
                                            className="w-full mt-[3px] 2xl:mt-[7px] rounded-[5px] placeholder-gray-700 outline-brand_color text-input_text 2xl:text-[15px] h-input_height 2xl:h-[45px] border-[1px] border-[#eee] p-[12px]"
                                          />
                                          {showLastActiveCalendar && (
                                            <div className="absolute z-10 bg-white shadow-md border-[1px] border-[#eee] w-full p-2">
                                              <DateRange ranges={tempLastActiveRange} onChange={handleLastActiveDateChange} months={1} rangeColors={["#4A90E2"]} />
                                              <div className="flex justify-end gap-2 mt-2">
                                                <button onClick={cancelLastActiveDate} className="px-3 py-1 border rounded bg-gray-300">Cancel</button>
                                                <button onClick={applyLastActiveDate} className="px-3 py-1 border rounded bg-indigo-600 text-white">Apply</button>
                                              </div>
                                            </div>
                                          )}
                                        </div>
              ...
              ${hoveredItem === option.label
                    ...
                    ${hoveredItem2 === option.label
                                      ...
                                      <div className="mb-[10px]">
                                        <label htmlFor="customerType" className='text-label_size 2xl:text-[16px] text-neutral-600 font-label_weight'>
                                          Auto Invoice
                                        </label>
                                        <br />
                                        <div ref={customerTypeDropdownRef} className="relative w-full mt-[5px]">
                                          ...
                                          <div
                                            ...
                                            className="flex items-center justify-between bg-white border border-[#ddd] rounded-[5px] text-input_text 2xl:text-[16px] h-input_height px-4 py-2 cursor-pointer hover:border-brand_color transition"
                                          >
            ...
            hover:bg-gray-100`}
                                                >
                                                  <span
                                                    className={`w-3 h-3 rounded-full ${hoveredItem === option.label ? `${option.bgColor}` : selectedCustomerType === option.label && !hoveredItem ? 'bg-white' : option.bgColor
                                                      }`}
                                                  ></span>
                                                  {option.label}
                                                </li>
                    ...
                    ${hoveredItem3 === option.label
                                      ...
                                      {
                                        companyinput == "" && designationinput == "" && createByInput == "" && publishByInput == "" && updateByInput == "" && selectedOption == "Select Authorized" && selectedCustomerType == "Select Invoice" && selectedOption2 == "Select Visibility" && selectedOption3 == "Select Status" ? "" : <button
                          ...
                          >
                            {columns
                              ...
                              .map((col) => (
                                <td
                                  key={col.key}
                                  className={`lg:px-4 py-4 text-sm w-full lg:w-[45%] px-[40px] text-nowrap ${col.key === "status"
  ...
  <label className="relative inline-flex items-center cursor-pointer">
    <input
      ...
      onChange={(e) =>
        handleToggleStatus(rowIndex, e.target.checked ? "Active" : "Inactive")
      }
    />
    <div
  ...
  >
    {row[col.key]}
                                    ...
                                    <div className="flex items-center space-x-3 w-[150px]">
                                      ...
                                      <label className="relative inline-flex items-center cursor-pointer">
                                        <input
                                          ...
                                          checked={row[col.key] === "Active"}
                                          onChange={(e) =>
                                            toggleStatus2(rowIndex, e.target.checked ? "Active" : "Deactive")
                                          }
                                        />
                                        <div
                                          className={`w-12 h-6 bg-red-500 dark:bg-gray-700 rounded-[2px] flex items-center px-1 transition-all duration-300 cursor-pointer peer-checked:bg-green-500 ${row[col.key] === "Active" ? "bg-green-500" : "bg-red-500"
                                            }`}
                                        >
                                          <div
                                            className={`w-5 h-5 bg-white rounded-[2px] shadow-md transform transition-all duration-300 ${row[col.key] === "Active" ? "translate-x-[20px]" : "translate-x-0"
                                              }`}
                                          ></div>
                                        </div>
                                      ...
                                      {/* Status Text */}
                                      <span
                                        className={`text-sm font-medium ${row[col.key] === "Active" ? "text-green-600" : "text-red-500"
                                          }`}
                                      >
                                        {row[col.key]}
                                      </span>
                                    ...
                                    <div className="flex justify-start items-center gap-[12px] relative">
                                      ...
                                      {/* Plan Access Button */}
                                      <div onClick={() => setDiscountPopupOpen(true)} className="relative w-[30px] h-[30px] border border-gray-300 rounded-md flex justify-center items-center text-gray-700 cursor-pointer hover:border-brand_color dark:border-gray-700 dark:hover:bg-gray-800 group">
                                        <SiBuymeacoffee />
                                      ...
                                      <NavLink to="/super-view-customer" className="w-[30px] h-[30px]  border border-gray-300 rounded-[5px] hover:text-brand_color flex justify-center items-center text-[15px] cursor-pointer hover:border-brand_color dark:border-gray-700 dark:hover:bg-gray-800 group relative">
                                        <GoEye />
                                        <span className="absolute hidden group-hover:block bottom-[35px] left-1/2 transform -translate-x-1/2 bg-brand_color text-white text-xs font-medium rounded px-2 py-0.5 z-10">
                                          View
                                          <span className="absolute left-1/2 transform -translate-x-1/2 bottom-[-5px] w-0 h-0 border-x-[5px] border-x-transparent border-t-[5px] border-brand_color"></span>
                                      ...
                                      <NavLink to="/super-edit-customer" className="w-[30px] h-[30px]  border border-gray-300 rounded-[5px] hover:text-brand_color flex justify-center items-center text-[15px] cursor-pointer hover:border-brand_color dark:border-gray-700 dark:hover:bg-gray-800 group relative">
                                        <CiEdit />
                                        <span className="absolute hidden group-hover:block bottom-[35px] left-1/2 transform -translate-x-1/2 bg-brand_color text-white text-xs font-medium rounded px-2 py-0.5 z-10">
                                          Edit
                                          <span className="absolute left-1/2 transform -translate-x-1/2 bottom-[-5px] w-0 h-0 border-x-[5px] border-x-transparent border-t-[5px] border-brand_color"></span>
                  ...
                  {discountPopupOpen && <Priceaccess isOpen={discountPopupOpen} onClose={() => setDiscountPopupOpen(false)} />}
