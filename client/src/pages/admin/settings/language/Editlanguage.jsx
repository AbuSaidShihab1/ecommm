...
const Editlanguage = () => {
  ...
  useEffect(() => {
    const fetchCountryData = async () => {
      try {
        const response = await axios.get(`${base_url}/super/admin/language/${id}`);
        ...
        if (response.data) {
          ...
          setFormData({
            ...
            publishedDate: new Date(country.publishedDate),
            image: country.image
          });
          setContent(country.description);
        }
      } catch (error) {
        console.error("Error fetching country data:", error);
        setToastMessage("Failed to load country data");
        setToastType("error");
      } finally {
        setIsFetching(false);
      }
  ...
  const validateForm = () => {
    ...
    if (!formData.languageName.trim()) {
      newErrors.countryName = "Language name is required";
      isValid = false;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(`${base_url}/super/admin/language/${id}`, countryData, {
        headers: {
      ...
      <section className={activesidebar ? 'w-[100%] h-[100vh] overflow-y-auto transition-all duration-300' : 'transition-all duration-300 w-[100%] overflow-y-auto xl:w-[85%] h-[100vh]'}>
        ...
        <section className='w-[100%] m-auto py-[20px] xl:py-[40px] px-[10px] xl:px-[30px]'>
          <div className='w-full flex justify-between items-center'>
            <div>
              <h1 className='text-[20px] font-[600] mb-[8px]'>Edit {formData.countryName}</h1>
              <ul className='flex justify-center items-center gap-[10px] text-neutral-500 text-[14px] font-[500]'>
                <li>Dashboard</li>
                <li><IoIosArrowForward /></li>
                <li>Setting</li>
                <li><IoIosArrowForward /></li>
                <li>{formData.languageName}</li>
              </ul>
            </div>
          </div>
                ...
                <div className="space-y-4 flex justify-start flex-col bg-white">
                  ...
                  <div className='px-[15px] py-[10px] space-y-4 flex justify-start flex-col'>
                    ...
                    <div className="flex items-center">
                      ...
                      <p className="text-sm text-gray-700">
                        Visibility: <span className="font-semibold">{formData.visibility}</span>
                        <button
                          onClick={() => {
