...
const Dateformat = () => {
  ...
  // --------------table coulms
  const [data,setData]=useState(dateformat)
  console.log("hello")
...
const columns = [
  { key: "id", label: "ID" },
  { key: "dateformat", label: "Dateformat	" },
...
const [selectedPublisher, setSelectedPublisher] = useState(null);
  ...
  const applyFilters = () => {
    ...
    const filtered = data.filter((item) => {
      ...
      const matchesSearchTerm = searchTerm ? item.timeformat.toLowerCase().includes(searchTerm.toLowerCase()) || item.description.toLowerCase().includes(searchTerm.toLowerCase()) || item.updateBy.toLowerCase().includes(searchTerm.toLowerCase()) ||  item.authorizedBy.toLowerCase().includes(searchTerm.toLowerCase()) || item.authorizedDate.toLowerCase().includes(searchTerm.toLowerCase()) || item.updateBy.toLowerCase().includes(searchTerm.toLowerCase()) || item.updateDate.toLowerCase().includes(searchTerm.toLowerCase()) ||  item.authorized.toLowerCase().includes(searchTerm.toLowerCase()) || item.visibility.toLowerCase().includes(searchTerm.toLowerCase())  || item.status.toLowerCase().includes(searchTerm.toLowerCase()) : true;
      // Create Date filter
  ...
  const handleDeleteTicket = async () => {
    ...
    try {
      const response = await fetch(`${base_url}/super/admin/timezone/${ticketToDelete}`, {
        method: 'DELETE',
        headers: {
      ...
      toast.error('Error deleting Timezone. Please try again.', {
        id: toastId,
        duration: 4000,
      });
    } finally {
    ...
    >
      <div className="p-6">
        ...
        <p className="text-gray-600 dark:text-gray-300 mt-2 mb-6">
          Are you sure you want to delete this Dateformat? This action cannot be undone and all
 ...
 <section className='w-[100%] m-auto py-[20px]  xl:py-[40px] px-[20px] lg:px-[30px]'>
     <div className='w-full flex md:justify-between items-center  md:flex-row flex-col justify-start'>
        ...
        {/* ---------------table --------------- */}
            ...
            <NavLink to="/super-new-date-format"className="w-[50%] md:w-auto ">
...
<div className="relative  sm:rounded-lg">
  ...
  <div className="relative w-full md:w-auto">
      ...
      <input
        ...
        placeholder="Search for dateformat"
        value={searchTerm}
                  ...
                  >
                    {col.key === "image" ? (
                     <div className='flex justify-start items-center gap-[10px] w-full pr-[50px]'>
                      ...
                      />
                       <div>
                                    {row.languageName.length > 40 ? <p className='text-[15px] font-[500] text-black'>{row.languageName.slice(0,40)}..</p>:<p className='text-[15px] font-[600] text-black'>{row.languageName}</p> }
                      ...
                      >
                        {row[col.key]}
                           ...
                           <div className="flex justify-start items-center gap-[12px] relative">
                                                 ...
                                                 <NavLink to={`/super-view-date-format/${row._id}`} className="w-[30px] h-[30px]  border border-gray-300 rounded-[5px] hover:text-brand_color flex justify-center items-center text-[15px] cursor-pointer hover:border-brand_color dark:border-gray-700 dark:hover:bg-gray-800 group relative">
                                                   <GoEye  />
                                   ...
                                   <span className="absolute left-1/2 transform -translate-x-1/2 bottom-[-5px] w-0 h-0 border-x-[5px] border-x-transparent border-t-[5px] border-brand_color"></span>
                                                 ...
                                                 <NavLink to={`/super-edit-date-format/${row._id}`} className="w-[30px] h-[30px]  border border-gray-300 rounded-[5px] hover:text-brand_color flex justify-center items-center text-[15px] cursor-pointer hover:border-brand_color dark:border-gray-700 dark:hover:bg-gray-800 group relative">
                                                   <CiEdit />
                                                   <span className="absolute hidden group-hover:block bottom-[35px] left-1/2 transform -translate-x-1/2 bg-brand_color text-white text-xs font-medium rounded px-2 py-0.5 z-10">
                                                     Edit
                                                       <span className="absolute left-1/2 transform -translate-x-1/2 bottom-[-5px] w-0 h-0 border-x-[5px] border-x-transparent border-t-[5px] border-brand_color"></span>
