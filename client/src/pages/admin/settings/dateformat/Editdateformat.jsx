...
const Editdateformat = () => {
  ...
  const [isEditing, setIsEditing] = useState(!!id); // True if we're editing, false if creating new
   ...
   // Visibility options
   const options = [
     { label: 'Approved', color: 'green', bg: 'bg-green-50' },
     { label: 'Pending', color: 'orange', bg: 'bg-orange-50' },
     { label: 'Rejected', color: 'red', bg: 'bg-red-50' }
   ];
  ...
  // Fetch timezone data if editing
  useEffect(() => {
    if (id) {
      const fetchTimezone = async () => {
        setIsLoading(true);
        try {
          const response = await axios.get(`${base_url}/super/admin/dateformat/${id}`);
          if (response) {
            ...
            setFormData({
              ...
              visibility: timezone.visibility,
              publishedDate: new Date(timezone.createdAt) // Assuming createdAt is the publish date
            });
          } else {
            setToastMessage(response.data.message || 'Failed to fetch dateformat');
            setToastType('error');
          }
        } catch (error) {
          console.error('Error fetching dateformat:', error);
          setToastMessage(error.response?.data?.message || 'Failed to fetch timezone');
          setToastType('error');
        } finally {
          setIsLoading(false);
        }
  ...
  const validateForm = () => {
    ...
    if (!formData.dateformat.trim()) {
      newErrors.dateformat = 'Timeformat name is required';
      valid = false;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (id) {
        ...
        const response = await axios.put(`${base_url}/super/admin/dateformat/${id}`, {
        ...
        setToastMessage('Dateformat updated successfully!');
        setToastType('success');
      } else {
        // Create new timezone
        const response = await axios.post(`${base_url}/super/admin/dateformat`, {
          dateformat: formData.dateformat,
          description: formData.description,
          status: formData.status,
          visibility: formData.visibility,
        ...
        setToastMessage('Dateformat created successfully!');
        setToastType('success');
      ...
      console.error('Error saving Dateformat:', error);
      setToastMessage(error.response?.data?.message || `Failed to ${id ? 'update' : 'create'} dateformat`);
      setToastType('error');
    } finally {
      setIsLoading(false);
    }
      ...
      <section className={activesidebar ? 'w-[100%] h-[100vh] overflow-y-auto transition-all duration-300':' transition-all duration-300 w-[100%] overflow-y-auto xl:w-[85%] h-[100vh]'}>
        ...
        <section className='w-[100%]  m-auto py-[20px] xl:py-[40px] px-[10px] xl:px-[30px]'>
          <div className='w-full flex justify-between items-center'>
            <div>
              <h1 className='text-[20px] font-[600] mb-[8px]'>{isEditing ? 'Edit Dateformat' : 'New Dateformat'}</h1>
              <ul className='flex justify-center items-center gap-[10px] text-neutral-500 text-[14px] font-[500]'>
                <li>Dashboard</li>
                <li><IoIosArrowForward/></li>
                <li>Setting</li>
                <li><IoIosArrowForward/></li>
                <li>{isEditing ? 'Edit Dateformat' : 'New Dateformat'}</li>
              </ul>
            </div>
          </div>
