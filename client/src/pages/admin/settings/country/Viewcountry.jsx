  ...
  useEffect(() => {
    const fetchCountry = async () => {
      try {
        ...
        setError(err.response?.data?.message || 'Failed to fetch country data');
        setLoading(false);
      }
    };
      ...
      <section className={activesidebar ? 'w-[100%] h-[100vh] overflow-y-auto transition-all duration-300' : 'transition-all duration-300 w-[100%] overflow-y-auto xl:w-[85%] h-[100vh]'}>
        ...
        <section className='w-[100%] m-auto py-[20px] xl:py-[40px] px-[10px] xl:px-[30px]'>
          <div className='w-full flex justify-between items-center'>
            <div>
              <h1 className='text-[20px] font-[600] mb-[8px]'> {country.languageName}</h1>
              <ul className='flex justify-center items-center gap-[10px] text-neutral-500 text-[14px] font-[500]'>
                <li>Dashboard</li>
                <li><IoIosArrowForward /></li>
                <li>Setting</li>
                <li><IoIosArrowForward /></li>
                <li> {country.languageName}</li>
              </ul>
            </div>
          </div>
          ...
          <div className='flex justify-center w-full flex-col xl:flex-row gap-[20px] mt-[20px]'>
            ...
            <section className='w-[100%] xl:w-[30%]'>
              ...
              {/* Image Section */}
              <div className="relative w-full h-40 mt-[10px] mb-[10px]">
                <div className="w-full h-full rounded-[5px] overflow-hidden border-2 border-gray-200">
                  {country.image ? (
                    <img
                      src={country.image}
                      alt="Country"
                    ...
                    <div className="flex items-center justify-center flex-col w-full h-full bg-gray-200 text-gray-500">
                      <p className='text-[14px] 2xl:text-[15px]'>No Image Available</p>
                    </div>
                  )}
                </div>
              </div>
