...
const Edittimeformat = () => {
      ...
      const fetchTimezone = async () => {
        ...
        try {
          ...
          if (response) {
            ...
            setFormData({
              timeformat: timezone.timeformat,
            ...
            setToastMessage(response.data.message || 'Failed to fetch timezone');
            setToastType('error');
          }
        } catch (error) {
  ...
  const validateForm = () => {
    ...
    if (!formData.timeformat.trim()) {
      newErrors.timezoneName = 'Timeformat name is required';
      valid = false;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (id) {
        ...
        const response = await axios.put(`${base_url}/super/admin/timeformat/${id}`, {
        ...
        setToastMessage('Timeformat updated successfully!');
        setToastType('success');
      } else {
        // Create new timezone
        const response = await axios.post(`${base_url}/super/admin/timeformat`, {
          timezoneName: formData.timezoneName,
          description: formData.description,
          status: formData.status,
          visibility: formData.visibility,
        ...
        setToastMessage('Timeformat created successfully!');
        setToastType('success');
      ...
      console.error('Error saving Timeformat:', error);
      setToastMessage(error.response?.data?.message || `Failed to ${id ? 'update' : 'create'} Timeformat`);
      setToastType('error');
    } finally {
      setIsLoading(false);
    }
      ...
      <section className={activesidebar ? 'w-[100%] h-[100vh] overflow-y-auto transition-all duration-300':' transition-all duration-300 w-[100%] overflow-y-auto xl:w-[85%] h-[100vh]'}>
        ...
        <section className='w-[100%]  m-auto py-[20px] xl:py-[40px] px-[10px] xl:px-[30px]'>
          <div className='w-full flex justify-between items-center'>
            <div>
              <h1 className='text-[20px] font-[600] mb-[8px]'>{isEditing ? 'Edit Timeformat' : 'New Timeformat'}</h1>
              <ul className='flex justify-center items-center gap-[10px] text-neutral-500 text-[14px] font-[500]'>
                <li>Dashboard</li>
                <li><IoIosArrowForward/></li>
                <li>Setting</li>
                <li><IoIosArrowForward/></li>
                <li>{isEditing ? 'Edit Timeformat' : 'New Timeformat'}</li>
              </ul>
            </div>
          </div>
