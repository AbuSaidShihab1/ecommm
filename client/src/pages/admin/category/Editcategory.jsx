...
const Editcategory = () => {
    ...
    const fetchCategory = async () => {
      try {
        ...
        if (response.data.success) {
          ...
          setProfileImage(category.image);
        }
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      const response = await axios.put(`${base_url}/super/admin/categories/${id}`, {
        name: formData.name,
      ...
      if (response.data.success) {
        ...
        showToast('Failed to update category: ' + response.data.message, 'error');
      }
    } catch (error) {
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        showToast('Category saved as draft successfully!');
        setTimeout(() => {
          navigate('/categories');
        }, 1500);
      }
            ...
            <form onSubmit={handleSubmit} className="pt-[20px]">
              ...
              <div className="w-[100%] mb-[20px]">
                ...
                <select
                  ...
                  onChange={handleChange}
                  className="w-full mt-[3px] 2xl:mt-[7px] rounded-[5px] placeholder-gray-700 outline-brand_color text-input_text 2xl:text-[15px] h-input_height 2xl:h-[45px] border-[1px] p-[12px] border-[#eee]"
                >
