...
const Categorylist = () => {
   const navigate=useNavigate();
     ...
     const { categories, loadingCategories, fetchCategories } = useSuper();
...
const [selectedPublisher, setSelectedPublisher] = useState(null);
  ...
  const applyFilters = () => {
    ...
    const filtered = data.filter((item) => {
      ...
      const matchesSearchTerm = searchTerm ? item.name.toLowerCase().includes(searchTerm.toLowerCase()) ||  item.description.toLowerCase().includes(searchTerm.toLowerCase()) || item.createBy.toLowerCase().includes(searchTerm.toLowerCase()) || item.createDate.toLowerCase().includes(searchTerm.toLowerCase()) ||  item.publishBy.toLowerCase().includes(searchTerm.toLowerCase()) || item.publishDate.toLowerCase().includes(searchTerm.toLowerCase()) || item.updateBy.toLowerCase().includes(searchTerm.toLowerCase()) || item.updateDate.toLowerCase().includes(searchTerm.toLowerCase()) ||  item.authorized.toLowerCase().includes(searchTerm.toLowerCase()) || item.visibility.toLowerCase().includes(searchTerm.toLowerCase())  || item.status.toLowerCase().includes(searchTerm.toLowerCase()) : true;
      // Create Date filter
...
const handleDeleteTicket = async () => {
  ...
  try {
    const response = await fetch(`${base_url}/super/admin/category/${ticketToDelete}`, {
      method: 'DELETE',
      headers: {
...
const toggleStatus = async (rowIndex, newStatus) => {
  try {
    const categoryId = data[rowIndex]._id;

    const response = await axios.patch(`${base_url}/super/admin/categories/${categoryId}/status`, {
      status: newStatus
    }, {
          headers: {
 ...
 <section className='w-[100%] m-auto py-[20px]  xl:py-[40px] px-[20px] lg:px-[30px]'>
     <div className='w-full flex md:justify-between items-center  md:flex-row flex-col justify-start'>
        <div className='w-full md:w-auto'>
              <h1 className='text-[20px] font-[600] mb-[8px]'>Category List</h1>
        ...
        {/* ---------------table --------------- */}
            ...
            <NavLink to="/super-new-business-category"className="w-[50%] md:w-auto ">
    ...
    >
      {row[col.key]}
                           ...
                           <div className="flex justify-start items-center gap-[12px] relative">
                                                 ...
                                                 <NavLink to={`/super-view-category/${row._id}`} className="w-[30px] h-[30px]  border border-gray-300 rounded-[5px] hover:text-brand_color flex justify-center items-center text-[15px] cursor-pointer hover:border-brand_color dark:border-gray-700 dark:hover:bg-gray-800 group relative">
                                                   <GoEye  />
                                   ...
                                   <span className="absolute left-1/2 transform -translate-x-1/2 bottom-[-5px] w-0 h-0 border-x-[5px] border-x-transparent border-t-[5px] border-brand_color"></span>
                                                 ...
                                                 <NavLink to={`/super-business-edit-category/${row._id}`} className="w-[30px] h-[30px]  border border-gray-300 rounded-[5px] hover:text-brand_color flex justify-center items-center text-[15px] cursor-pointer hover:border-brand_color dark:border-gray-700 dark:hover:bg-gray-800 group relative">
                                                   <CiEdit />
                                                   <span className="absolute hidden group-hover:block bottom-[35px] left-1/2 transform -translate-x-1/2 bg-brand_color text-white text-xs font-medium rounded px-2 py-0.5 z-10">
                                                     Edit
                                                       <span className="absolute left-1/2 transform -translate-x-1/2 bottom-[-5px] w-0 h-0 border-x-[5px] border-x-transparent border-t-[5px] border-brand_color"></span>
