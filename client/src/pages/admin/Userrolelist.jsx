...
const [selectedPublisher, setSelectedPublisher] = useState(null);
  ...
  const applyFilters = () => {
    ...
    const filtered = data.filter((item) => {
      ...
      const matchesSearchTerm = searchTerm ?   item.roleName.toLowerCase().includes(searchTerm.toLowerCase()) || item.updatedBy.toLowerCase().includes(searchTerm.toLowerCase())  || item.authorized.toLowerCase().includes(searchTerm.toLowerCase()) || item.description.toLowerCase().includes(searchTerm.toLowerCase()) ||  item.createdDate.toLowerCase().includes(searchTerm.toLowerCase()) || item.createdBy.toLowerCase().includes(searchTerm.toLowerCase()) ||  item.updatedDate.toLowerCase().includes(searchTerm.toLowerCase()) || item.status.toLowerCase().includes(searchTerm.toLowerCase()) : true;
...
const handleDeleteTicket = async () => {
  ...
  try {
    const response = await fetch(`${base_url}/super/admin/delete-user-role/${ticketToDelete}`, {
      method: 'DELETE',
      headers: {
    ...
    if (data.success) {
      ...
      toast.error(data.message || 'Failed to delete User Role ', {
        id: toastId,
        duration: 4000,
      });
    }
     ...
     <div className='w-full flex md:justify-between items-center  md:flex-row flex-col justify-start'>
        ...
        {/* ---------------table --------------- */}
          <div className='w-full   md:w-auto flex gap-[10px] md:m-0 mt-[15px]'>
               ...
               <NavLink to="/super-new-user-role"className="w-[50%] md:w-auto ">
                      ...
                      <span
                        ...
                        {row[col.key]}
                          ...
                          <div className="flex justify-start items-center gap-[12px] relative">
                                                         ...
                                                         <NavLink to={`/users/super-edit-user-role/${row._id}`} className="w-[30px] h-[30px]  border border-gray-300 rounded-[5px] hover:text-brand_color flex justify-center items-center text-[15px] cursor-pointer hover:border-brand_color dark:border-gray-700 dark:hover:bg-gray-800 group relative">
                                                           <CiEdit />
