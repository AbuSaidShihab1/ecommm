  ...
  return (
    <div className="fixed inset-0 flex items-center z-[100000000000]   overflow-auto py-[40px] justify-center bg-black bg-opacity-50">
     ...
     <div className="bg-white rounded-lg w-[80%] h-auto  xl:w-[60%] 2xl:w-[50%] shadow-lg">
      ...
      <h1 className='text-[15px] 2xl:text-[18px] font-[500] 2xl:font-semibold text-gray-600'>Edit Package</h1>
...
const DiscountPopup = ({ isOpen, onClose, product, onApplyDiscount }) => {
  ...
  useEffect(() => {
    ...
    // Restore previous values when reopening
    const savedDiscount = JSON.parse(localStorage.getItem("discountData"));
    if (savedDiscount) {
      setDiscountType(savedDiscount.discountType);
      setDiscountAmount(savedDiscount.discountAmount);
      setCouponCode(savedDiscount.couponCode);
      setCouponDiscount(savedDiscount.couponDiscount);
      setDescription(savedDiscount.description);
    }
  }, [isOpen]);
  ...
  const handleApply = () => {
    ...
    // Save values so they persist on reopen
    const discountData = {
      discountType,
      discountAmount,
      couponCode,
...
const TaxPopup = ({ isOpen, onClose, product, onApplyTax }) => {
  ...
  const handleApplyTax = (e) => {
    e.preventDefault();
    if(taxType==="Exclusive"){
      const totalTax = standardTax + customTax;
  ...
  return (
    <section className='w-full h-[100vh] flex font-poppins'>
       ...
       <div className='w-full flex justify-between items-center'>
        <div>
              <h1 className='text-[20px] lg:text-[20px] font-[600] mb-[8px]'>New Payment:              </h1>
...
<div className="w-full mb-[20px] relative">
  <label className="text-label_size 2xl:text-[16px] font-label_weight 2xl:font-[500] text-neutral-600">Search Package</label>
  <input
    type="text"
    value={searchTerm}
...
<div>
      ...
      <table className="min-w-full divide-y divide-gray-200 w-full overflow-x-auto border-[1px] border-[#eee] dark:divide-gray-700">
        <thead className="bg-gray-100 dark:bg-gray-800">
          <tr className='uppercase'>
            ...
            <th className="px-4 py-2 text-left text-sm text-nowrap font-medium text-gray-700">Package</th>
            ...
            <th className="px-4 py-2 text-left text-sm text-nowrap font-medium text-gray-700">Duration</th>
  ...
  {selectedProducts.map((row, index) => {
    ...
    console.log(row.tax)
    const totalAmount =( subtotal + row.tax)-row.discount;
    ...
    return (
      <tr key={row.id} className="dark:hover:bg-gray-800">
        ...
        <td className="text-nowrap px-[20px]">
          <div className="w-full bg-gray-200 border flex justify-between items-center">
            <div
              onClick={() => handleInputChange({ target: { value: row.quantity - 1 } }, "quantity")}
              className="bg-gray-200 cursor-pointer p-2 flex items-center justify-center w-[30%]"
            >
              <LuMinus />
            </div>
            <input
              ...
              className="bg-white px-2 py-1  text-center w-[40%] border outline-none [&::-webkit-inner-spin-button]:appearance-none 
  ...
  [&::-moz-number-spin-box]:hidden"
          />
             ...
             <td className="p-2">
                         <p className='flex justify-center items-center gap-[10px]  '><span className='text-[17px]'>{row.discount.toFixed(0,2)}</span> <span   onClick={() => {
...
<DiscountEditorPopup
      ...
      <div className="w-full mb-[20px]">
        ...
        <tr>
              ...
              <th className="px-4 py-2 text-left text-nowrap uppercase text-sm font-medium text-table_title dark:text-gray-300">Payment Method</th>
              <th className="px-4 py-2 text-left text-nowrap uppercase text-sm font-medium text-table_title dark:text-gray-300">Duration</th>
              <th className="px-4 py-2 text-left text-nowrap uppercase text-sm font-medium text-table_title dark:text-gray-300">Transaction ID</th>
              <th className="px-4 py-2 text-left text-nowrap uppercase text-sm font-medium text-table_title dark:text-gray-300">Total Amount</th>
              <th className="px-4 py-2 text-left text-nowrap uppercase text-sm font-medium text-table_title dark:text-gray-300">Action</th>
            </tr>
        </thead>
          ...
          <tbody>
            {/* Example Row */}
            <tr>
              <td className="border border-[#eee] p-[10px]">15-Jan-2023</td>
              <td className="border border-[#eee] p-[10px]">Credit Card</td>
              <td className="border border-[#eee] p-[10px]">5</td>
              <td className="border border-[#eee] p-[10px]">4332sdSvfg435</td>
              <td className="border border-[#eee] p-[10px]">4332sdSvfg435</td>
              <td className="border border-[#eee] p-[10px]">200</td>
...
export default Pricenewpayment